...
export class MemoryStream extends Stream {
    ...
    private _capacity: number;     // length of usable portion of buffer for stream
    // Note that _capacity == _buffer.Length for non-user-provided byte[]'s

    // private bool _expandable;  // User-provided buffers aren't expandable.
    private _writable: boolean;    // Can user write to this stream?
    // private readonly bool _exposable;   // Whether the array can be returned to the user.
    private _isOpen: boolean;      // Is this stream open or closed?

    // private CachedCompletedInt32Task _lastReadTask; // The last successful task returned from ReadAsync

    private static MemStreamMaxLength = Number.MAX_SAFE_INTEGER;
    ...
    public constructor(buffer: Uint8Array, index: number | undefined = undefined, count: number | undefined = undefined, writable: boolean = false) {
        ...
        Throw.ThrowIfNull(buffer);
        Throw.ThrowIfNegative(index);
    ...
    public override Seek(offset: number, loc: SeekOrigin): number {
        ...
        switch (loc) {
            case SeekOrigin.Begin:
                return this.SeekCore(offset, this._origin);
            case SeekOrigin.Current:
                return this.SeekCore(offset, this._position);
            case SeekOrigin.End:
                return this.SeekCore(offset, this._length);
            default:
                throw new Error("Argument_InvalidSeekOrigin");
        }
    }
    ...
    private SeekCore(offset: number, loc: number): number {
        ...
        const tempPosition = loc + Number(offset);
        if (loc + Number(offset) < this._origin || tempPosition < this._origin)
            throw new Error("IO_SeekBeforeBegin");
    ...
    private EnsureNotClosed() {
        if (!this._isOpen) {
            throw new Error("ObjectDisposedException_StreamClosed");
        }
    }
    ...
    // public override ReadBytes(count: number) : number[]
    // {
    //     this.EnsureNotClosed();

    //     let origPos = this._position;
    //     let newPos = origPos + count;
    ...
    //     var span = this._buffer.slice(origPos, count);
    ...
    public override Read(buffer: Uint8Array, offset: number, count: number): number {
        ...
        assert(this._position + n >= 0);  // len is less than 2^31 -1.
