...
export abstract class MemoryBlockProvider {
    ...
    /// <exception cref="IOException">Error while reading from the memory source.</exception>
    public GetMemoryBlock(start?: number, size?: number): AbstractMemoryBlock {
        start = start ?? 0;
        size = size ?? this.Size;
    ...
    /// The operations on the stream has to be done under a lock of <see cref="StreamConstraints.GuardOpt"/> if non-undefined.
