...
//         //
//         // Note that for a top level type, the namespace is string.Empty, *not* undefined (as Reflection surfaces it.)
//         // This is a concession to the fact that MetadataReader's fast String equal methods don't accept undefined.
//         //
//         public static string AppendTypeName(this string ns, string name)
//         {
//             Debug.Assert(ns != undefined, "For top level types, the namespace must be string.Empty, not undefined");
...
//             int count = genericMethodArguments == undefined ? 0 : genericMethodArguments.Length;
//             if (count != 0)
//             {
//                 sb.Append('[');
...
//
export function ToRoAssemblyName(assemblyName: AssemblyName): RoAssemblyName {
    if (assemblyName.Name == undefined) {
        Throw.ArgumentException("assemblyName");
    }
    ...
    const pkt = assemblyName.GetPublicKeyToken()?.slice() ?? new Uint8Array(16);
